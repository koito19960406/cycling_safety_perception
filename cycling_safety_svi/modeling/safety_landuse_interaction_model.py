...
This script extends the best choice model with safety * landuse interaction effects.
It takes a trained model (pickle file) and adds interaction terms between safety scores 
and land use categories (buildenvironment).
...
Usage:
    ...
    python safety_landuse_interaction_model.py --model_path path/to/best_model.pickle --main_design_path path/to/main_design.csv
"""
...
class SafetyLanduseInteractionModel:
    """Extends a trained choice model with safety * landuse interaction effects"""
    ...
    def __init__(self, model_path='reports/models/choice_20250606_162138/best_model_top_35_with_safety.pickle', main_design_path='data/raw/main_design.csv', 
        ...
        print(f"Safety-Landuse Interaction Model")
    ...
    def create_landuse_dummy_variables(self, data_subset):
        """Create dummy variables for land use categories"""
        
        print("Creating land use dummy variables...")
        ...
        for category in self.landuse_categories:
            if category != reference_category:
                ...
                data_subset[f'safety_landuse_{category}_1'] = data_subset['safety_score_1'] * data_subset[f'landuse_{category}_1']
                data_subset[f'safety_landuse_{category}_2'] = data_subset['safety_score_2'] * data_subset[f'landuse_{category}_2']
        
        print(f"Created dummy variables for land use categories (reference: {reference_category})")
        print(f"Created safety * landuse interaction terms")
        
        return data_subset, reference_category
    
    def estimate_interaction_model(self, train_data, test_data=None):
        """Estimate the safety * landuse interaction model"""
        
        print("Estimating Safety * Landuse Interaction Model")
        print("=" * 50)
        
        # Prepare the modeling data with landuse variables
        train_model_data, reference_category = self.create_landuse_dummy_variables(train_data)
        ...
        for category in self.landuse_categories:
            if category != reference_category:
                interaction_cols.extend([f'safety_landuse_{category}_1', f'safety_landuse_{category}_2'])
        ...
        train_database = db.Database('safety_landuse_interaction_train', train_model_data)
        ...
        # Add safety * landuse interaction effects
        interaction_params = {}
        for category in self.landuse_categories:
            if category != reference_category:
                param_name = f"B_SAFETY_LANDUSE_{category.upper()}"
                interaction_params[category] = Beta(param_name, 0, None, None, 0)
                
                interaction_var1 = train_database.variables[f'safety_landuse_{category}_1']
                interaction_var2 = train_database.variables[f'safety_landuse_{category}_2']
        ...
        train_results = self._estimate_mnl(V1, V2, CHOICE, train_database, 'safety_landuse_interaction')
        ...
        model_info = {
            ...
            'model_components': {
                ...
                'safety_landuse_interactions': list(interaction_params.keys()),
    ...
    def _evaluate_on_test_data(self, train_results, test_data, reference_category, seg_features_used, all_cols):
        ...
        test_model_data, _ = self.create_landuse_dummy_variables(test_data_copy)
        ...
        for idx, row in test_model_data.iterrows():
            ...
            for category in self.landuse_categories:
                if category != reference_category:
                    param_name = f"B_SAFETY_LANDUSE_{category.upper()}"
                    if param_name in param_dict:
                        V1 += param_dict[param_name] * row[f'safety_landuse_{category}_1']
                        V2 += param_dict[param_name] * row[f'safety_landuse_{category}_2']
    ...
    def analyze_interaction_effects(self, results):
        """Analyze and interpret the safety * landuse interaction effects"""
        
        print("Analyzing safety * landuse interaction effects...")
        ...
        for category in self.landuse_categories:
            if category != self.model_info['reference_landuse_category']:
                interaction_param = f"B_SAFETY_LANDUSE_{category.upper()}"
    ...
    def _create_summary_report(self, train_results, interaction_effects, train_metrics, test_metrics=None):
        ...
        with open(report_path, 'w') as f:
            f.write("SAFETY * LANDUSE INTERACTION MODEL REPORT\n")
            f.write("=" * 50 + "\n\n")
            ...
            f.write("SAFETY EFFECTS BY LAND USE:\n")
            ...
            f.write(f"  Safety*Landuse interactions: {', '.join(self.model_info['model_components']['safety_landuse_interactions'])}\n")
    ...
    def run_analysis(self):
        ...
        print("Starting safety * landuse interaction analysis...")
        ...
        print("✓ Safety * landuse interaction analysis completed!")
        print(f"Results saved to: {self.output_dir}")
...
def main():
    """Main function to run safety * landuse interaction analysis"""
    
    parser = argparse.ArgumentParser(description='Add safety * landuse interaction effects to choice model')
    ...
    interaction_model = SafetyLanduseInteractionModel(
