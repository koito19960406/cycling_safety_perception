"""
Vegetation-Landuse Interaction Model

This script extends the best choice model with vegetation * landuse interaction effects.
It takes a trained model (pickle file) and adds interaction terms between vegetation scores 
...
Usage:
    python vegetation_landuse_interaction_model.py --model_path path/to/best_model.pickle
    python vegetation_landuse_interaction_model.py --model_path path/to/best_model.pickle --main_design_path path/to/main_design.csv
"""
...
class VegetationLanduseInteractionModel:
    """Extends a trained choice model with vegetation * landuse interaction effects"""
    ...
    def __init__(self, model_path='reports/models/choice_20250606_162138/best_model_top_35_with_safety.pickle', 
                 ...
                 output_dir='reports/models/vegetation_interaction'):
        """
        ...
        self.output_dir = Path(output_dir) / f"vegetation_landuse_{timestamp}"
        self.output_dir.mkdir(parents=True, exist_ok=True)
        
        print(f"Vegetation-Landuse Interaction Model")
    ...
    def load_and_prepare_data(self, cv_dcm_path='data/raw/cv_dcm.csv',
        ...
        # Load segmentation data (we need this for vegetation)
    ...
    def _merge_datasets_with_landuse(self):
        ...
        # Add segmentation features (including vegetation)
        ...
        # Print vegetation statistics if available
        if 'Vegetation_1' in merged_data.columns:
            print(f"\nVegetation statistics:")
            print(f"  Alternative 1: mean={merged_data['Vegetation_1'].mean():.4f}, std={merged_data['Vegetation_1'].std():.4f}")
            print(f"  Alternative 2: mean={merged_data['Vegetation_2'].mean():.4f}, std={merged_data['Vegetation_2'].std():.4f}")
    
    def create_vegetation_landuse_dummy_variables(self, data_subset):
        """Create dummy variables for land use categories and vegetation interaction terms"""
        
        print("Creating vegetation * land use interaction variables...")
        
        # Use 'Woongebied' as reference category (most common residential area)
        reference_category = 'Recreatie'
        
        # Ensure vegetation data exists
        if 'Vegetation_1' not in data_subset.columns or 'Vegetation_2' not in data_subset.columns:
            print("Warning: Vegetation data not found. Creating zero vegetation columns.")
            data_subset['Vegetation_1'] = 0
            data_subset['Vegetation_2'] = 0
        ...
        for category in self.landuse_categories:
            if category != reference_category:
                ...
                # Create interaction terms with vegetation
                data_subset[f'vegetation_landuse_{category}_1'] = data_subset['Vegetation_1'] * data_subset[f'landuse_{category}_1']
                data_subset[f'vegetation_landuse_{category}_2'] = data_subset['Vegetation_2'] * data_subset[f'landuse_{category}_2']
        
        print(f"Created dummy variables for land use categories (reference: {reference_category})")
        print(f"Created vegetation * landuse interaction terms")
        
        return data_subset, reference_category
    
    def estimate_interaction_model(self, train_data, test_data=None):
        """Estimate the vegetation * landuse interaction model (excluding safety)"""
        
        print("Estimating Vegetation * Landuse Interaction Model")
        print("=" * 50)
        
        # Prepare the modeling data with landuse and vegetation variables
        train_model_data, reference_category = self.create_vegetation_landuse_dummy_variables(train_data)
        
        # Required columns for the base model (excluding safety)
        required_cols = ['CHOICE', 'TL1', 'TT1', 'TL2', 'TT2', 'Vegetation_1', 'Vegetation_2']
        ...
        for category in self.landuse_categories:
            if category != reference_category:
                interaction_cols.extend([f'vegetation_landuse_{category}_1', f'vegetation_landuse_{category}_2'])
        
        # Initialize segmentation columns (excluding vegetation since it's already in the main model)
        seg_cols = []
        seg_features_used = []
        ...
        if hasattr(self, 'original_model_features') and len(self.original_model_features) > 0 and self.segmentation_data is not None:
            ...
            # Map original feature names to column names and check availability (excluding vegetation)
            for feature in self.original_model_features:
                # Skip vegetation since it's already included as main effect
                if feature.lower() in ['vegetation']:
                    continue
        ...
        train_database = db.Database('vegetation_landuse_interaction_train', train_model_data)
        ...
        vegetation1, vegetation2 = train_database.variables['Vegetation_1'], train_database.variables['Vegetation_2']
        ...
        B_VEGETATION = Beta('B_VEGETATION', 0, None, None, 0)
        
        # Start with base utility
        V1_components = [B_TL * TL1 / 3, B_TT * TT1 / 10, B_VEGETATION * vegetation1]
        V2_components = [B_TL * TL2 / 3, B_TT * TT2 / 10, B_VEGETATION * vegetation2]
        ...
        # Add vegetation * landuse interaction effects
        interaction_params = {}
        for category in self.landuse_categories:
            if category != reference_category:
                param_name = f"B_VEGETATION_LANDUSE_{category.upper()}"
                interaction_params[category] = Beta(param_name, 0, None, None, 0)
                
                interaction_var1 = train_database.variables[f'vegetation_landuse_{category}_1']
                interaction_var2 = train_database.variables[f'vegetation_landuse_{category}_2']
        ...
        train_results = self._estimate_mnl(V1, V2, CHOICE, train_database, 'vegetation_landuse_interaction')
        ...
        model_info = {
            ...
            'model_components': {
                'base_effects': ['TL', 'TT', 'VEGETATION'],
                'landuse_main_effects': list(landuse_params.keys()),
                'vegetation_landuse_interactions': list(interaction_params.keys()),
    ...
    def _evaluate_on_test_data(self, train_results, test_data, reference_category, seg_features_used, all_cols):
        ...
        test_model_data, _ = self.create_vegetation_landuse_dummy_variables(test_data_copy)
        ...
        for idx, row in test_model_data.iterrows():
            ...
            # Add vegetation terms
            V1 += param_dict.get('B_VEGETATION', 0) * row['Vegetation_1']
            V2 += param_dict.get('B_VEGETATION', 0) * row['Vegetation_2']
            ...
            for category in self.landuse_categories:
                if category != reference_category:
                    param_name = f"B_VEGETATION_LANDUSE_{category.upper()}"
                    if param_name in param_dict:
                        V1 += param_dict[param_name] * row[f'vegetation_landuse_{category}_1']
                        V2 += param_dict[param_name] * row[f'vegetation_landuse_{category}_2']
    ...
    def analyze_interaction_effects(self, results):
        """Analyze and interpret the vegetation * landuse interaction effects"""
        
        print("Analyzing vegetation * landuse interaction effects...")
        ...
        interaction_effects = {}
        main_vegetation_effect = estimated_params.loc['B_VEGETATION', 'Value'] if 'B_VEGETATION' in estimated_params.index else 0
        
        for category in self.landuse_categories:
            if category != self.model_info['reference_landuse_category']:
                interaction_param = f"B_VEGETATION_LANDUSE_{category.upper()}"
                if interaction_param in estimated_params.index:
                    interaction_coef = estimated_params.loc[interaction_param, 'Value']
                    total_vegetation_effect = main_vegetation_effect + interaction_coef
                    
                    interaction_effects[category] = {
                        'interaction_coefficient': interaction_coef,
                        'main_vegetation_effect': main_vegetation_effect,
                        'total_vegetation_effect': total_vegetation_effect,
        ...
        interaction_effects[self.model_info['reference_landuse_category']] = {
            ...
            'main_vegetation_effect': main_vegetation_effect,
            'total_vegetation_effect': main_vegetation_effect,
            'p_value': None,
            't_stat': None
        }
    ...
    def _create_summary_report(self, train_results, interaction_effects, train_metrics, test_metrics=None):
        ...
        report_path = self.output_dir / 'vegetation_interaction_model_report.txt'
        
        with open(report_path, 'w') as f:
            f.write("VEGETATION * LANDUSE INTERACTION MODEL REPORT\n")
            f.write("=" * 50 + "\n\n")
            ...
            f.write("VEGETATION EFFECTS BY LAND USE:\n")
            for category, effects in interaction_effects.items():
                f.write(f"  {category}:\n")
                f.write(f"    Total vegetation effect: {effects['total_vegetation_effect']:.6f}\n")
            ...
            f.write(f"  Vegetation*Landuse interactions: {', '.join(self.model_info['model_components']['vegetation_landuse_interactions'])}\n")
    ...
    def run_analysis(self):
        """Run the complete vegetation * landuse interaction analysis"""
        
        print("Starting vegetation * landuse interaction analysis...")
        ...
        print("✓ Vegetation * landuse interaction analysis completed!")
        print(f"Results saved to: {self.output_dir}")
...
def main():
    """Main function to run vegetation * landuse interaction analysis"""
    
    parser = argparse.ArgumentParser(description='Add vegetation * landuse interaction effects to choice model')
    ...
    parser.add_argument('--output_dir', type=str, default='reports/models/vegetation_interaction',
    ...
    interaction_model = VegetationLanduseInteractionModel(
    ...
    # Print summary
    print("\n" + "="*60)
    print("SUMMARY OF VEGETATION * LANDUSE INTERACTION EFFECTS")
    print("="*60)
    
    for category, effects in interaction_effects.items():
        print(f"{category}: Total vegetation effect = {effects['total_vegetation_effect']:.6f}")
        if effects['interaction_coefficient'] != 0:
            significance = ""
            if effects['p_value'] is not None:
