"""
BikeLane-Landuse Interaction Model (Mixed Logit Version)

This script extends a trained choice model with bike lane * land use interaction effects
using a mixed logit (MXL) formulation.

Usage:
    python bikelane_landuse_interaction_model.py --model_path path/to/final_model.pickle
"""
...
class BikeLaneLanduseInteractionModel:
    """Extends a trained choice model with bike lane * landuse interaction effects using MXL."""
    ...
    def __init__(self, model_path, main_design_path='data/raw/main_design.csv', output_dir='reports/models/interaction'):
        ...
        print(f"BikeLane-Landuse Interaction Model (MXL)")
    ...
    def load_and_prepare_data(self, cv_dcm_path='data/raw/cv_dcm.csv',
        ...
        # Always load segmentation data for this model as it needs 'Bike Lane'
    ...
    def _merge_datasets_with_landuse(self):
        ...
        for _, row in self.main_design.iterrows():
            if pd.notna(row['alt1_imageid']):
                img_to_landuse[row['alt1_imageid'] + '.jpg'] = row['alt1_buildenvironment']
            if pd.notna(row['alt2_imageid']):
                img_to_landuse[row['alt2_imageid'] + '.jpg'] = row['alt2_buildenvironment']
        
        merged_data['landuse_1'] = merged_data['IMG1'].map(img_to_landuse)
        merged_data['landuse_2'] = merged_data['IMG2'].map(img_to_landuse)
        
        self.landuse_categories = merged_data['landuse_1'].dropna().unique().tolist()
        ...
        # Ensure 'Bike Lane' and other features from base model are present
        features_to_add = self.original_model_features
        if 'Bike Lane' not in features_to_add:
            features_to_add.append('Bike Lane')
    ...
    def create_bikelane_landuse_interactions(self, data):
        """Create interaction terms for bike lane presence and land use categories."""
        print("Creating bike lane * landuse interaction terms...")
        reference_category = 'Woongebied'
        if reference_category not in self.landuse_categories:
            reference_category = self.landuse_categories[0]
        
        interaction_features = []
        main_effect_features = []
        
        # Main effects of land use
        for category in self.landuse_categories:
            if category != reference_category:
                ...
                # Interaction term: Bike Lane * Land Use Dummy
                interact_name = f"bikelane_X_landuse_{category}"
                interaction_features.append(interact_name)
                data[f'{interact_name}_1'] = data['Bike_Lane_1'] * data[f'{main_effect_name}_1']
                data[f'{interact_name}_2'] = data['Bike_Lane_2'] * data[f'{main_effect_name}_2']
                
        return data, main_effect_features, interaction_features

    def estimate_interaction_model(self):
        """Estimate the MXL bike lane * landuse interaction model."""
        print("\nEstimating Bike Lane * Landuse Interaction Model (MXL)...")
        
        train_data = self.merged_data[self.merged_data['train'] == 1].copy()
        test_data = self.merged_data[self.merged_data['test'] == 1].copy()
        
        train_data, main_effects, interaction_effects = self.create_bikelane_landuse_interactions(train_data)
        test_data, _, _ = self.create_bikelane_landuse_interactions(test_data)
        
        # Ensure Bike Lane main effect is included
        model_features = [f for f in self.original_model_features if f != 'Bike Lane']
        
        features = model_features + main_effects + interaction_effects + ['SAFETY_SCORE', 'Bike Lane']
        ...
        fixed_params = {f: Beta(f"B_{f.replace(' ', '_').replace('___', ' - ')}", 0, None,None,0) 
                        for f in fixed_features if f not in random_params_config}
        ...
        for q in range(obs_per_ind):
            ...
            for name, param in fixed_params.items():
                v1_name, v2_name = f"{name.replace(' ', '_').replace('___', ' - ')}_1_{q}", f"{name.replace(' ', '_').replace('___', ' - ')}_2_{q}"
                if v1_name in biodata_wide.variables:
                    V1 += param * Variable(v1_name)
                    V2 += param * Variable(v2_name)
            V.append({1: V1, 2: V2})

        results = estimate_mxl(V, {1:1, 2:1}, 'CHOICE', obs_per_ind, self.num_draws, biodata_wide, 'bikelane_landuse_interaction', self.output_dir)
        _, test_biodata_wide, _ = prepare_panel_data(test_data, self.individual_id, 'CHOICE', features)
        test_sim_results = simulate_mxl(V, {1:1,2:1}, 'CHOICE', obs_per_ind, self.num_draws, test_biodata_wide, results.get_beta_values(), 'bikelane_landuse_interaction')
    ...
    def generate_results_table(self):
        ...
        lines = [
            "\\begin{table}[htbp]", "\\centering", "\\caption{BikeLane-Landuse Interaction Model (MXL)}",
            "\\label{tab:bikelane_landuse_interaction}", "\\resizebox{0.8\\textwidth}{!}{%",
        ...
        latex_content = "\n".join(lines)
        table_path = self.output_dir / 'bikelane_landuse_interaction_model.tex'
...
def main():
    parser = argparse.ArgumentParser(description='Run BikeLane-Landuse Interaction Model (MXL)')
    ...
    interaction_model = BikeLaneLanduseInteractionModel(model_path=args.model_path)
