"""
Safety-Roadtype Interaction Model (Mixed Logit Version)

This script extends a trained choice model with safety * road type interaction effects
using a mixed logit (MXL) formulation.

Usage:
    python safety_roadtype_interaction_model.py --model_path path/to/final_model.pickle
"""
...
class SafetyRoadtypeInteractionModel:
    """Extends a trained choice model with safety * road type interaction effects using MXL."""
    ...
    def __init__(self, model_path, main_design_path='data/raw/main_design.csv', output_dir='reports/models/interaction'):
        ...
        print(f"Safety-Roadtype Interaction Model (MXL)")
        print(f"Base model path: {self.model_path}")
        print(f"Output directory: {self.output_dir}")
        
        logging.getLogger('biogeme').setLevel(logging.WARNING)

        # Assumed based on common Dutch road types, will be confirmed from data
        self.roadtype_categories = ['Fietspad', 'Fietsstrook', 'Woonerf', 'Erftoegangsweg', 'Gebiedsontsluitingsweg']
    ...
    def _merge_datasets_with_roadtype(self):
        ...
        for _, row in self.main_design.iterrows():
            if pd.notna(row['alt1_imageid']):
                img_to_roadtype[row['alt1_imageid'] + '.jpg'] = row['alt1_wegtype']
            if pd.notna(row['alt2_imageid']):
                img_to_roadtype[row['alt2_imageid'] + '.jpg'] = row['alt2_wegtype']
        
        merged_data['roadtype_1'] = merged_data['IMG1'].map(img_to_roadtype)
        merged_data['roadtype_2'] = merged_data['IMG2'].map(img_to_roadtype)
        
        # Update roadtype categories from data and fill NaNs
        self.roadtype_categories = merged_data['roadtype_1'].dropna().unique().tolist()
        print(f"Discovered road types from data: {self.roadtype_categories}")
        most_common = merged_data['roadtype_1'].mode()[0]
        merged_data['roadtype_1'].fillna(most_common, inplace=True)
        merged_data['roadtype_2'].fillna(most_common, inplace=True)
    ...
    def create_roadtype_dummy_variables(self, data):
        """Create dummy variables for road type categories and safety interactions."""
        print("Creating dummy variables and interaction terms for road type...")
        reference_category = 'Fietspad'  # Cycle path as reference
        if reference_category not in self.roadtype_categories:
            reference_category = self.roadtype_categories[0] # Fallback
        ...
        for category in self.roadtype_categories:
            if category != reference_category:
                main_effect_name = f'roadtype_{category}'
                main_effect_features.append(main_effect_name)
                data[f'{main_effect_name}_1'] = (data['roadtype_1'] == category).astype(int)
                data[f'{main_effect_name}_2'] = (data['roadtype_2'] == category).astype(int)

                interact_name = f"safety_roadtype_{category}"
    ...
    def estimate_interaction_model(self):
        """Estimate the MXL safety * road type interaction model."""
        print("\nEstimating Safety * Roadtype Interaction Model (MXL)...")
        
        train_data = self.merged_data[self.merged_data['train'] == 1].copy()
        test_data = self.merged_data[self.merged_data['test'] == 1].copy()
        
        train_data, main_effects, interaction_effects = self.create_roadtype_dummy_variables(train_data)
        test_data, _, _ = self.create_roadtype_dummy_variables(test_data)
        ...
        for q in range(obs_per_ind):
            ...
            # Fixed parameters (segmentation, roadtype main effects, interactions)
        ...
        results = estimate_mxl(V, {1:1, 2:1}, 'CHOICE', obs_per_ind, self.num_draws, biodata_wide, 'roadtype_interaction', self.output_dir)
        _, test_biodata_wide, _ = prepare_panel_data(test_data, self.individual_id, 'CHOICE', all_features)
        test_sim_results = simulate_mxl(V, {1:1,2:1}, 'CHOICE', obs_per_ind, self.num_draws, test_biodata_wide, results.get_beta_values(), 'roadtype_interaction')
    ...
    def generate_results_table(self):
        ...
        lines = [
            "\\begin{table}[htbp]",
            "    \\centering",
            "    \\caption{Safety-Roadtype Interaction Model (MXL)}",
            "    \\label{tab:roadtype_interaction}",
        ...
        latex_content = "\n".join(lines)
        table_path = self.output_dir / 'roadtype_interaction_model.tex'
        with open(table_path, 'w') as f: f.write(latex_content)
        print(f"LaTeX table saved to {table_path}")

    def run_analysis(self):
        """Run the complete safety * road type interaction analysis."""
...
def main():
    parser = argparse.ArgumentParser(description='Run Safety-Roadtype Interaction Model (MXL)')
    ...
    interaction_model = SafetyRoadtypeInteractionModel(model_path=args.model_path)
