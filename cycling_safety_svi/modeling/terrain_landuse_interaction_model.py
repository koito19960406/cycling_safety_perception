"""
Terrain-Landuse Interaction Model

This script extends the best choice model with terrain * landuse interaction effects.
It takes a trained model (pickle file) and adds interaction terms between terrain characteristics 
...
Usage:
    python terrain_landuse_interaction_model.py --model_path path/to/best_model.pickle
    python terrain_landuse_interaction_model.py --model_path path/to/best_model.pickle --main_design_path path/to/main_design.csv
"""
...
class TerrainLanduseInteractionModel:
    """Extends a trained choice model with terrain * landuse interaction effects"""
    ...
    def __init__(self, model_path='reports/models/choice_20250606_162138/best_model_top_35_with_safety.pickle', 
                 ...
                 output_dir='reports/models/terrain_interaction'):
        """
        ...
        self.output_dir = Path(output_dir) / f"terrain_landuse_{timestamp}"
        self.output_dir.mkdir(parents=True, exist_ok=True)
        
        print(f"Terrain-Landuse Interaction Model")
    ...
    def load_and_prepare_data(self, cv_dcm_path='data/raw/cv_dcm.csv',
        ...
        # Load segmentation data (we need this for terrain)
    ...
    def _merge_datasets_with_landuse(self):
        ...
        # Add segmentation features (including terrain)
        ...
        # Print terrain statistics if available
        if 'Terrain_1' in merged_data.columns:
            print(f"\nTerrain statistics:")
            print(f"  Alternative 1: mean={merged_data['Terrain_1'].mean():.4f}, std={merged_data['Terrain_1'].std():.4f}")
            print(f"  Alternative 2: mean={merged_data['Terrain_2'].mean():.4f}, std={merged_data['Terrain_2'].std():.4f}")
    
    def create_terrain_landuse_dummy_variables(self, data_subset):
        """Create dummy variables for land use categories and terrain interaction terms"""
        
        print("Creating terrain * land use interaction variables...")
        
        # Use 'Woongebied' as reference category (most common residential area)
        reference_category = 'Recreatie'
        
        # Ensure terrain data exists
        if 'Terrain_1' not in data_subset.columns or 'Terrain_2' not in data_subset.columns:
            print("Warning: Terrain data not found. Creating zero terrain columns.")
            data_subset['Terrain_1'] = 0
            data_subset['Terrain_2'] = 0
        ...
        for category in self.landuse_categories:
            if category != reference_category:
                ...
                # Create interaction terms with terrain
                data_subset[f'terrain_landuse_{category}_1'] = data_subset['Terrain_1'] * data_subset[f'landuse_{category}_1']
                data_subset[f'terrain_landuse_{category}_2'] = data_subset['Terrain_2'] * data_subset[f'landuse_{category}_2']
        
        print(f"Created dummy variables for land use categories (reference: {reference_category})")
        print(f"Created terrain * landuse interaction terms")
        
        return data_subset, reference_category
    
    def estimate_interaction_model(self, train_data, test_data=None):
        """Estimate the terrain * landuse interaction model (excluding safety)"""
        
        print("Estimating Terrain * Landuse Interaction Model")
        print("=" * 50)
        
        # Prepare the modeling data with landuse and terrain variables
        train_model_data, reference_category = self.create_terrain_landuse_dummy_variables(train_data)
        
        # Required columns for the base model (excluding safety)
        required_cols = ['CHOICE', 'TL1', 'TT1', 'TL2', 'TT2', 'Terrain_1', 'Terrain_2']
        ...
        for category in self.landuse_categories:
            if category != reference_category:
                interaction_cols.extend([f'terrain_landuse_{category}_1', f'terrain_landuse_{category}_2'])
        
        # Initialize segmentation columns (excluding terrain since it's already in the main model)
        seg_cols = []
        seg_features_used = []
        ...
        if hasattr(self, 'original_model_features') and len(self.original_model_features) > 0 and self.segmentation_data is not None:
            ...
            # Map original feature names to column names and check availability (excluding terrain)
            for feature in self.original_model_features:
                # Skip terrain since it's already included as main effect
                if feature.lower() in ['terrain']:
                    continue
        ...
        train_database = db.Database('terrain_landuse_interaction_train', train_model_data)
        ...
        terrain1, terrain2 = train_database.variables['Terrain_1'], train_database.variables['Terrain_2']
        ...
        B_TERRAIN = Beta('B_TERRAIN', 0, None, None, 0)
        
        # Start with base utility
        V1_components = [B_TL * TL1 / 3, B_TT * TT1 / 10, B_TERRAIN * terrain1]
        V2_components = [B_TL * TL2 / 3, B_TT * TT2 / 10, B_TERRAIN * terrain2]
        ...
        # Add terrain * landuse interaction effects
        interaction_params = {}
        for category in self.landuse_categories:
            if category != reference_category:
                param_name = f"B_TERRAIN_LANDUSE_{category.upper()}"
                interaction_params[category] = Beta(param_name, 0, None, None, 0)
                
                interaction_var1 = train_database.variables[f'terrain_landuse_{category}_1']
                interaction_var2 = train_database.variables[f'terrain_landuse_{category}_2']
        ...
        train_results = self._estimate_mnl(V1, V2, CHOICE, train_database, 'terrain_landuse_interaction')
        ...
        model_info = {
            ...
            'model_components': {
                'base_effects': ['TL', 'TT', 'TERRAIN'],
                'landuse_main_effects': list(landuse_params.keys()),
                'terrain_landuse_interactions': list(interaction_params.keys()),
    ...
    def _evaluate_on_test_data(self, train_results, test_data, reference_category, seg_features_used, all_cols):
        ...
        test_model_data, _ = self.create_terrain_landuse_dummy_variables(test_data_copy)
        ...
        for idx, row in test_model_data.iterrows():
            ...
            V1 += param_dict.get('B_TERRAIN', 0) * row['Terrain_1']
            V2 += param_dict.get('B_TERRAIN', 0) * row['Terrain_2']
            for category in self.landuse_categories:
                if category != reference_category:
                    param_name = f"B_LANDUSE_{category.upper()}"
            ...
            for category in self.landuse_categories:
                if category != reference_category:
                    param_name = f"B_TERRAIN_LANDUSE_{category.upper()}"
                    if param_name in param_dict:
                        V1 += param_dict[param_name] * row[f'terrain_landuse_{category}_1']
                        V2 += param_dict[param_name] * row[f'terrain_landuse_{category}_2']
            for feature in seg_features_used:
                param_name = f"B_{feature.replace(' ', '_').replace('-', '_').upper()}"
                if param_name in param_dict:
    ...
    def analyze_interaction_effects(self, results):
        """Analyze and interpret the terrain * landuse interaction effects"""
        print("Analyzing terrain * landuse interaction effects...")
        try:
            ...
            try:
                ...
                return {}
        interaction_effects = {}
        main_terrain_effect = estimated_params.loc['B_TERRAIN', 'Value'] if 'B_TERRAIN' in estimated_params.index else 0
        for category in self.landuse_categories:
            if category != self.model_info['reference_landuse_category']:
                interaction_param = f"B_TERRAIN_LANDUSE_{category.upper()}"
                if interaction_param in estimated_params.index:
                    interaction_coef = estimated_params.loc[interaction_param, 'Value']
                    total_terrain_effect = main_terrain_effect + interaction_coef
                    interaction_effects[category] = {
                        'interaction_coefficient': interaction_coef,
                        'main_terrain_effect': main_terrain_effect,
                        'total_terrain_effect': total_terrain_effect,
        ...
        interaction_effects[self.model_info['reference_landuse_category']] = {
            ...
            'main_terrain_effect': main_terrain_effect,
            'total_terrain_effect': main_terrain_effect,
            'p_value': None,
            't_stat': None
        }
    ...
    def _create_summary_report(self, train_results, interaction_effects, train_metrics, test_metrics=None):
        ...
        report_path = self.output_dir / 'terrain_interaction_model_report.txt'
        
        with open(report_path, 'w') as f:
            f.write("TERRAIN * LANDUSE INTERACTION MODEL REPORT\n")
            f.write("=" * 50 + "\n\n")
            ...
            f.write("TERRAIN EFFECTS BY LAND USE:\n")
            for category, effects in interaction_effects.items():
                f.write(f"  {category}:\n")
                f.write(f"    Total terrain effect: {effects['total_terrain_effect']:.6f}\n")
            ...
            f.write(f"  Terrain*Landuse interactions: {', '.join(self.model_info['model_components']['terrain_landuse_interactions'])}\n")
    ...
    def run_analysis(self):
        """Run the complete terrain * landuse interaction analysis"""
        
        print("Starting terrain * landuse interaction analysis...")
        ...
        print("✓ Terrain * landuse interaction analysis completed!")
        print(f"Results saved to: {self.output_dir}")
...
def main():
    """Main function to run terrain * landuse interaction analysis"""
    
    parser = argparse.ArgumentParser(description='Add terrain * landuse interaction effects to choice model')
    ...
    parser.add_argument('--output_dir', type=str, default='reports/models/terrain_interaction',
    ...
    interaction_model = TerrainLanduseInteractionModel(
    ...
    # Print summary
    print("\n" + "="*60)
    print("SUMMARY OF TERRAIN * LANDUSE INTERACTION EFFECTS")
    print("="*60)
    
    for category, effects in interaction_effects.items():
        print(f"{category}: Total terrain effect = {effects['total_terrain_effect']:.6f}")
        if effects['interaction_coefficient'] != 0:
            significance = ""
            if effects['p_value'] is not None:
