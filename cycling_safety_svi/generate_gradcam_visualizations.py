...
def overlay_heatmap(image, heatmap, alpha=0.5):
    ...
    # Blend images
    overlayed = alpha * image + (1 - alpha) * heatmap_colored
        ...
        for batch in tqdm(dataloader, desc="Generating Grad-CAM visualizations"):
            ...
            for i in range(len(images)):
                try:
                    ...
                    plt.axis('off')
                    plt.title(f'Grad-CAM Heatmap: {image_name}')
                    plt.colorbar()
                    plt.savefig(heatmap_path, bbox_inches='tight', dpi=150)
                    plt.close()
                    
                    # Create overlay
                    overlayed = overlay_heatmap(single_image, cam, alpha=0.5)
                    ...
                    plt.axis('off')
                    plt.title(f'Grad-CAM Overlay: {image_name}')
...
def main():
    ...
    parser.add_argument(
        '--batch_size', 
        type=int, 
        default=8,  # Smaller batch size for Grad-CAM
    ...
    dataloader = DataLoader(dataset, 
                            ...
                            shuffle=False,
                            num_workers=2)  # Reduced for Grad-CAM
